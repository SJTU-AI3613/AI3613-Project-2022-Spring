...
int main(int argc, char *argv[]) {
    ...
    if (with_predicate) {
        ...
        auto person_id_eq_salary_id =
            std::make_unique<query::BinaryExpr>(query::BinaryOperator::Eq, std::move(person_id), std::move(salary_id));

        predicate = std::move(person_id_eq_salary_id);
    }

    // SELECT * FROM Person JOIN Salary ON Person.id = Salary.id;
    auto join = std::make_unique<query::PhysicalNestedLoopJoin>(
        &join_schema, std::move(seq_scan1), std::move(seq_scan2), std::move(predicate));

    query::ExecutionEngine engine(&bm, &catalog);
    auto result = engine.execute(join.get());

    std::vector<std::vector<type::Value>> expect;
    if (with_predicate) {
        expect = make_vector(
        ...
        expect = make_vector(
            make_vector(type::Value(0), type::Value(20, "Alice"), type::Value(17), type::Value(2), type::Value(100)),
            make_vector(type::Value(0), type::Value(20, "Alice"), type::Value(17), type::Value(3), type::Value(200)),
            make_vector(type::Value(1), type::Value(20, "Bob"), type::Value(18), type::Value(2), type::Value(100)),
            make_vector(type::Value(1), type::Value(20, "Bob"), type::Value(18), type::Value(3), type::Value(200)),
            make_vector(type::Value(2), type::Value(20, "Carol"), type::Value(19), type::Value(2), type::Value(100)),
            make_vector(type::Value(2), type::Value(20, "Carol"), type::Value(19), type::Value(3), type::Value(200)),
            make_vector(type::Value(3), type::Value(20, "Dave"), type::Value(20), type::Value(2), type::Value(100)),
            make_vector(type::Value(3), type::Value(20, "Dave"), type::Value(20), type::Value(3), type::Value(200)));
    }
